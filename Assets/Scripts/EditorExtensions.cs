...
namespace RPG.Editor
{
    public static class EditorExtensions
    {
        ...
        public static LinkedList<T> FindAllAssetsByType<T>(SearchOption options = SearchOption.AllDirectories, string additionalPath = "") where T : UnityEngine.Object
        {
            var pathes = Directory.GetFiles(Application.dataPath + additionalPath, "*.asset", options);
            var assets = new LinkedList<T>();

            foreach (var path in pathes)
            {
                var assetPath = "Assets" + path.Replace(Application.dataPath, "").Replace('\\', '/');
                var asset = AssetDatabase.LoadAssetAtPath(assetPath, typeof(UnityEngine.Object)) as T;
                if (asset != null) assets.AddLast(asset);
            }
